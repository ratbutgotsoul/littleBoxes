/*
 *  Development by: Nikolaos P. Kallitsis
...
(function($) { 
    $.fn.littleBoxes = function( options ){     
        ...
        var $mom=null,$boxes=null,$suggestions=null,$input=null,$output=$(this),stopTyping=null,loading=null,tmp='',options=$.extend(true,{},defaults,options);
        ...
        $mom.addClass(options['containerClass']).append(
            ...
            $loading=$('<span>',{'class':'loading'}).hide(), 
        ...
        $suggestions.add=function(s){ 
            ...
            if(!s.id){s.id=s.box;}
            $(this).append($suggestion=$('<li>',{'html':s.suggestion,'class':options['suggestionClass']}).data({'id':s.id,'box-layout':s.box}).click(function(){
                $boxes.add($(this));
            }).mouseover(function(){
        ...
        $input.keydown(function(e){
            ...
            tmp=value;
            $suggestions.clear();
            if(value==''){$loading.hide();return;}
            $.ajax({
                url:options['apiUrl'],
                data:{'search':value,'current':$mom.ids.join('+')}
            }).done(function(r){
                r=$.parseJSON(r);
        ...
        }}).blur(function(){$suggestions.clear();});
